...
void QASMparser::checkHeader(void) {
	std::istream* tmp_in = new std::ifstream (fname, std::ifstream::in);;
	QASMscanner* tmp_scan = new QASMscanner(*tmp_in);
	bool found_openqasm = false;
	bool found_include = false;
	Token tk;

	do {
		tk = tmp_scan->next();
		if(tk.kind == Token::Kind::openqasm) {
			found_openqasm = true;
		} else if(tk.kind == Token::Kind::include) {
			found_include = true;
		}
	} while (tk.kind != Token::Kind::eof);

	delete tmp_scan;
	delete tmp_in;

	if(found_openqasm == false) {
		printf("ERROR: undefined OPENQASM 2.0\n");
		exit(0);
	} else if(found_include == false) {
		printf("ERROR: undefined include\n");
		exit(0);
	}
}
...
void QASMparser::check(Token::Kind expected) {
	if (sym == expected) {
		...
		printf("ERROR: expected '%s' in line-%d\n", Token::KindNames[expected].c_str(), t.line);
		exit(0);
	}
}
...
std::pair<int, int> QASMparser::QASMargumentQreg() {
	...
	if(qregs.find(s) == qregs.end()) {
		printf("ERROR: argument is not a qreg in line-%d\n", t.line);
		exit(0);
	}
	...
	if(sym == Token::Kind::lbrack) {
		...
		if(offset < 0 || offset >= qregs[s].second) {
			printf("ERROR: index of qreg is out of bound in line-%d\n", t.line);
			exit(0);
		}
...
std::pair<std::string, int> QASMparser::QASMargumentCreg() {
	...
	if(cregs.find(s) == cregs.end()) {
		printf("ERROR: argument is not a creg in line-%d\n", t.line);
		exit(0);
	}
	...
	if(sym == Token::Kind::lbrack) {
		...
		if(index < 0 || index >= cregs[s].first) {
			printf("ERROR: index of creg is out of bound in line-%d\n", t.line);
			exit(0);
		}
		check(Token::Kind::rbrack);
	}
...
QASMparser::Expr* QASMparser::QASMexponentiation() {
	...
	if(sym == Token::Kind::real) {
		...
		printf("ERROR: unexpected numerical argument in line=%d\n", t.line);	
		exit(0);
	}
	return NULL;
}
...
void QASMparser::QASMgate(bool execute) {
	if(sym == Token::Kind::ugate) {
		...
		std::cout << "Applied gate: " << gate_name << std::endl;
			...
			printf("ERROR: undefined gate or code string in line-%d\n", t.line);
			exit(0);
		}
	}
}
	...
	while(sym != Token::Kind::rbrace) {
		if(sym == Token::Kind::ugate) {
			...
			if(compoundGates.find(name) == compoundGates.end()) {
				printf("ERROR: invalid gate in line=%d\n", t.line);
				exit(0);
			};
	...
	if(sym == Token::Kind::ugate || sym == Token::Kind::cxgate || sym == Token::Kind::identifier) {
		...
		if(execute) {
			...
			if(qreg.second == creg_size) {
				if(creg_size == 1) {
					cregs[creg.first].second[creg.second] = M(QReg, qreg.first);
				} else {
					for(int i = 0; i < creg_size; i++) {
						cregs[creg.first].second[i] = M(QReg, qreg.first+i);
					}
				}
...
void QASMparser::Parse() 
{
...
#if 0
	do {
		printf("t.str=%-10s, line=%02d, col=%02d, val=%d, kind=%-10s -> sym=%s\n", t.str.c_str(), t.line, t.col, t.val, t.kindStr(), t.kindStr(sym));
		scan();
	...
	do {
		if(sym == Token::Kind::qreg) {
			...
			if(it == cregs.end()) {
				printf("line=%d invalid creg in if statement\n", t.line);
				exit(0);
			} else {
				int creg_num = 0;
...
void QASMparser::get_cregStr(vector<string> &cregStr) {
	...
	for(auto creg : cregs) {
		char creg_str[128] = "";
		int creg_num = 0;
		for(int i = creg.second.first-1; i >= 0; i--) {
			creg_num = (creg_num << 1) | (creg.second.second[i] & 1);
		}
